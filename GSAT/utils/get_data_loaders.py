...
def get_data_loaders(data_dir, dataset_name, batch_size, splits, random_state, mutag_x=False):
    ...
    if dataset_name == 'ba_2motifs':
        ...
        # cnt_g = dataset.data.y.size(0)
        # cnt_node = dataset.data.x.size(0)
        # cnt_edge = dataset.data.edge_index.size(1)
        # print(f"Avg Number of Nodes: {cnt_node/cnt_g}")
        # print(f"Avg Number of Edges: {cnt_edge/cnt_g}")
        # print(f"Number of Classes: {dataset.data.y.max()+1}")
        # print(f"Number of Graphs: {cnt_g}")
    ...
    # cnt_g = (train_loader.dataset.data.y.size(0) + valid_loader.dataset.data.y.size(0) + test_loader.dataset.data.y.size(0))
    # cnt_node = (train_loader.dataset.data.x.size(0) + valid_loader.dataset.data.x.size(0) + test_loader.dataset.data.x.size(0))
    # cnt_edge = (train_loader.dataset.data.edge_index.size(1) + valid_loader.dataset.data.edge_index.size(1) +
    #             test_loader.dataset.data.edge_index.size(1))
    # print(f"Avg Number of Nodes: {cnt_node/cnt_g}")
    # print(f"Avg Number of Edges: {cnt_edge/cnt_g}")
    # print(f"Number of Classes: {train_loader.dataset.data.y.max()+1}")
    # print(f"Number of Graphs: {cnt_g}")
